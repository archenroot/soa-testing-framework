...
//        }
    ...
    private final FlowExecutionListener execListener = new FlowExecutionListener() {
    ...
    private String[] getListOfEnvironments() {
        ...
        return new String[]{"Ladislav Jech LocalHost", "vlado-localdev","kroky-localdev", "dev", "dev2", "dev3", "pre-sit", "sit", "uat", "prod"};
    }

    private void executeFramework() {
        clearAll();
        FlowExecutor flowExecutor = buildFlowExecutor();
    ...
    private void updateDetails(GUIObjects.Interface iface) {
        ...
        final InterfaceConfiguration ICFG = FCFG.getInterfaceConfig(name);
        DefaultMutableTreeNode nodeInterface = new DefaultMutableTreeNode(iface);
        DefaultMutableTreeNode nodeProjects = new DefaultMutableTreeNode("Projects");
        ...
        DefaultMutableTreeNode nodePatterns = new DefaultMutableTreeNode("Patterns");
        ...
        for (IfaceFlowPattern iFlowPattern : ICFG.getIfaceFlowPatterns()) {
            ...
            for (IfaceTestScenario iTestScenario : iFlowPattern.getIfaceTestScenario()) {
                ...
                for (IfaceExecBlock iExecutionBlock : iTestScenario.getIfaceExecBlock()) {
                    ...
                    ExecutionBlock executionBlock = MCFG.getExecutionBlock(iFlowPattern.getRefId(), iTestScenario.getRefId(), iExecutionBlock.getRefId());
                    for (Operation operation : executionBlock.getOperation()) {
                        DefaultMutableTreeNode nodeOperation = new DefaultMutableTreeNode(new GUIObjects.Operation(operation));
    ...
    private void updateLastRow(FlowExecutionEvent evt) {
        Result result = new Result(evt.getOperation().getName().value());
        result.setMessage(evt.getComponentResult().getResultMessage());
        result.setSuccess(evt.getComponentResult().isOverallResultSuccess());
        jtResultsModel.updateLastRow(result);
    }
    
    private void addRow(FlowExecutionEvent evt) {
        jtResultsModel.addRow(new Result(evt.getOperation().getName().value()));
    }

    private FlowExecutor buildFlowExecutor() {
        ...
        while (node != null) {
            ...
            if (userObject instanceof GUIObjects.Operation) {
                flowExecutor.setOperationName(((GUIObjects.Operation)userObject).getWrappedObject().getName().value());
            } else if (userObject instanceof GUIObjects.InterfaceExecutionBlock) {
                flowExecutor.setInterfaceExecutionBlockId(((GUIObjects.InterfaceExecutionBlock)userObject).getWrappedObject().getRefId());
            } else if (userObject instanceof GUIObjects.InterfaceTestScenario) {
                flowExecutor.setInterfaceTestScenarioId(((GUIObjects.InterfaceTestScenario)userObject).getWrappedObject().getRefId());
            } else if (userObject instanceof GUIObjects.IfaceFlowPattern) {
                flowExecutor.setInterfaceFlowPatternId(((GUIObjects.IfaceFlowPattern)userObject).getWrappedObject().getRefId());
            }
    ...
    private void displayResultDetails() {
        ...
        String str = jtResultsModel.getValueAt(rowIdx, 0).toString();
        StringBuilder sb = new StringBuilder("Operation: ").append(str).append("\n");
        
        str = jtResultsModel.getValueAt(rowIdx, 2).toString();
        sb.append("Status: ").append(str).append("\n");
        
        sb.append("\n");
        
        str = jtResultsModel.getValueAt(rowIdx, 1).toString();
        sb.append("Message:\n").append(str);
        
        JDialog dialog = new ResultsDetailDialog(this, true, sb.toString());

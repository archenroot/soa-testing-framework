...
import static com.ibm.soatf.config.FrameworkConfiguration.FLOW_PATTERN_DIR_NAME_PREFIX;
import static com.ibm.soatf.config.FrameworkConfiguration.IFACE_CONFIG_FILENAME;
import static com.ibm.soatf.config.FrameworkConfiguration.OSB_REFERENCE_PROJECT_DIR_NAME_PREFIX;
...
public class DirectoryStructureManager {
    ...
    public static void checkFrameworkDirectoryStructure() {
        
            MasterConfiguration masterConfig = ConfigurationManager.getInstance().getMasterConfig();
            final ListIterator<Interface> interfaces = masterConfig.getXmlConfig().getInterfaces().getInterface().listIterator();
    ...
    private static void validateIfaceStructure(Interface iface) {
        FrameworkConfiguration frameworkConfig = ConfigurationManager.getInstance().getFrameworkConfig();
        MasterConfiguration masterConfig = ConfigurationManager.getInstance().getMasterConfig();
            ...
            if (interfaceConfigFile.exists()) {
                try {
                    ...
                    while (ifaceFlowPatternIterator.hasNext()) {
                        ...
                        final File testScenarioNameDirectory = new File(patternDirectory, frameworkConfig.getValidFileSystemObjectName(ifaceFlowPattern.getTestName()));
